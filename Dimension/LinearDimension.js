...
class LinearDimension {
    ...
    constructor(
        ...
        let DimVec = Vector.CreateFromTwoPoint(
            (new Line(this.StartPoint, this.EndPoint)).VerticalPointOnLine(this.PointOnDim),
            this.PointOnDim
        );
    ...
    ContinueLabel(...Points) {
        ...
        for (let i = 0; i < Points.length; i++) {
            ...
            if (Points[i].DistTo(this.#LabelPositions[0]) < Points[i].DistTo(this.#LabelPositions[0])) {
                //Undeveloping
            }
            else {
                //Undeveloping
    ...
    static CreateDimensionWithVector(
        ...
        let TempN = Math.sqrt(Math.pow(Math.cos(Theta) * DistStEd, 2) / ExVector.length());
        let TempEd;
        ...
        let LabelToEndPt = Vector.CreateFromTwoPoint(EndPoint, DIM.#LabelPositions[1]);
        LabelToEndPt = LabelToEndPt.Normalise();
        let TempEnd = EndPoint.ClonePt().Add(LabelToEndPt.Multiple(Style.parameters.DIMDist));
        let NewLNEd = new Line(DIM.#LabelPositions[1], TempEnd).ExtendLine(Style.parameters.DIMDist, 0);
        DIM.Geoms["Line"].pop();
    ...
    #constructParaGeoms(DIMVector) {
        ...
        const DIMBar = (new Line(LabelSt.ClonePt(), LabelEd.ClonePt())).ExtendLine(Mul, Mul);
        ...
        let FinalVec = Vector.CreateFromTwoPoint(LabelSt.ClonePt(), LabelEd.ClonePt()).Normalise();
    ...
    static AddInstanceLinearDimension(scene, LNDim) {
        ...
        if (!LNDim.#IsContinue) {
            let Board = new TextBoard(LNDim.Content[0], 256, 128, LNDim.Style, LNDim.TextLocation[0]);
